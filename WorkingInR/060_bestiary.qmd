...
Note that `print()` shows the contents of the string, including the backslashes, while `cat()` shows what would be shown by ggplot etc.
...
```{r}
x <- 10
# is x between 0 and 20?
x > 0 | x < 20
# is x an odd number between 0 and 20? Wrong order of evaluation.
x > 0 | x < 20  & x %% 2 == 1 
# Brackets force correct order.
(x < 20 | x > 0)  & x %% 2 == 1 
```
...
```{r}
#| label: fig-missing-comma
#| fig.cap: > 
#|   Rstudio puts a cross in the margin and underlines the unexpected text
#|   after the missing comma.
#| fig-alt: Screenshot showing how RStudio responds to a missing comma.
knitr::include_graphics("figures/missing_comma.png")
```
...
```{r formula}
lm(bill_len ~ species + year + species:year, data = penguins)
```

This model can also be run with the following formulae with more concise notation.

```{r formula2, eval = FALSE}
lm(bill_len ~ species * year, data = penguins)
lm(bill_len ~ (species + year) ^ 2, data = penguins)
```
...
`ggplot2` uses `+` to add elements of the a plot together, `patchwork` use `+` to add plots together.


```{r}
p_box <- ggplot(penguins, aes(x = species, y = bill_len)) +
  geom_boxplot() +
...
p_violin <- ggplot(penguins, aes(x = species, y = bill_len)) +
  geom_violin() +
  labs(x = "Species", y = "Bill length mm")
