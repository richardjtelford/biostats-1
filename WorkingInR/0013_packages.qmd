...
If you want to install a package published on [GitHub](https://github.com/){target="_blank"}, you may use the function `pak::pak()` (install `pak` from CRAN first).
...
pak::pak(repo = "gavinsimpson/ggvegan")
```
...
# install.packages("pak")
pak::pak("biostats-r/biostats.tutorials")
```
...
Install `tidyverse`, and `pak` from CRAN and `biostats.tutorials` from GitHub using the code above.
:::
