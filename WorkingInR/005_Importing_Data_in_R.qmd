...
We have made some files available which you can download by running the following chunk: 

```{r}
#| label: download
#| eval: false
# make function to download data
get_from_github <- function(repo, branch = "main", newdir = "data") {
  # check Rstudio project activated
  if (is.null(rstudioapi::getActiveProject())) {
    stop("You should be using an RStudio project")
  }

  # download data into your project
  url <- paste0("https://github.com/", repo, "/archive/", branch, ".zip")
  destfile <- paste0(basename(repo), "-", branch, ".zip")
  ...
  # rename directory
  file.rename(tools::file_path_sans_ext(destfile), newdir)

  # remove the zip file
  file.remove(destfile)
}

get_from_github("biostats-r/biostats-data", newdir = "data")
```
...
Both `readr` and base R have functions for importing text files.
They have different arguments, different defaults and behave differently, so be careful not to mix them up.
`readr` functions have an underscore separating the words whereas base R uses a dot. 
:::
...
#| echo: true
#| message: true
library(tidyverse) # readr is part of the tidyverse
