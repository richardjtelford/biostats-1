...
library(patchwork)
# Jitter plot example
plot3 <- ggplot(data = penguins, aes(x = species, y = body_mass)) +
...
# Histogram example
plot2 <- ggplot(data = penguins, aes(x = flipper_len)) +
...
To make datasets in packages available you can either load the package with `library()` first, or use `data()`.

```{r data, error = TRUE}
sleepstudy # not available yet
data(sleepstudy, package = "lme4")
sleepstudy # available now
```
